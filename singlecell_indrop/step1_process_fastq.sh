...
SAMPLE_FILE=$1
OUTPUT_DIR=$2
TRANSFORM_FILE=$HOME/packages/umis/umis/examples/inDrop/transform_v2.json # Make this $3 later to accomodate for changing protocols
while read line_all
do
	...
	echo """
	umis fastqtransform --dual_index --separate_cb --cores 4 ${TRANSFORM_FILE} ${FASTQ_READ1}  ${FASTQ_READ2} > ${OUTPUT_DIR}/${SAMPLE_NAME}.parsed.fastq
	umis cb_filter --bc1 /data/langenau/singlecell_prkdc/gel_barcode1_list_revComp.txt --bc2 /data/langenau/singlecell_prkdc/gel_barcode2_list_revComp.txt ${OUTPUT_DIR}/${SAMPLE_NAME}.parsed.fastq > ${OUTPUT_DIR}/${SAMPLE_NAME}.processed.fastq
